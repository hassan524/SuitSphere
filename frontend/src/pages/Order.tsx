...
const Order = () => {
  ...
  const calculateDaysLeft = (orderDate: string): string => {
    ...
    const hoursLeft = Math.ceil(timeLeft / (1000 * 60 * 60)); // Convert to hours
    const daysLeft = Math.floor(hoursLeft / 24); // Convert to days
    if (daysLeft >= 1) {
    ...
    <div className="flex flex-col gap-[3.5rem] py-10 px-8 md:px-20 lg:px-32">
      ...
      <div className="flex flex-col gap-14">
        ...
        {Orders?.length > 0 ? (
          ...
          <p className="text-gray-500 text-center text-lg">You have no orders yet.</p>
        )}
      </div>

      {/* Order Summary Section */}
      <div className="flex flex-col gap-6 text-lg p-6">
        {/* <h2 className="text-2xl font-bold uppercase text-gray-700 text-center">Order Summary</h2>  */}
        {Orders?.length > 0 ? (
          Orders.map((order, orderIndex) => {
          ...
          <p className="text-gray-500 text-center text-lg">No orders available for summary.</p>
        )}
      </div>
    </div>
  );
